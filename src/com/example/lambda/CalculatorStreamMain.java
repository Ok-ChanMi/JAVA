...
public class CalculatorStreamMain {
    public static void main(String[] args) {
        ...
        Calculator3 addition = CalculatorVerMain::sum;


        // 연산을 테스트
        double result = addition.sum(100, 2, 3, 4, 5 );
        // 결과 값이 100을 초과하는 경우에만 100을 출력
        Arrays.stream(new double[]{result})
                .map(num -> Math.min(num, 100))
                .forEach(System.out::println);
        //스트림을 사용하여 sum() 메서드 결과 값이 100초과하는 경우만 100을 출력하도록 구현, 결과값을 스트림에 담고,
        //map() 메서드를 사용하여 결과값이 100이 초과하는 경우 100으로 값을 대체하고 , forEach() 메서드를 사용하여 최종 결과를 출력하는 코드
        //map(): 스트림내의 각 요소에 대해 특정 작업을 수행하는 함수를 적용하여 새로운 스트림을 생성하는 역할
        //그래서 map()이 스트림에 있는 각 요소를 확인하고, 그 값이 100을 초과하는 경우에는 100으로 값을 변경하는 작업을 수행
        //예시에 114는 100을 초과하기 때문에 Math.min(114, 100)의 결과는 100이 됨. 이 100을 새로운 스트림에 저장되어 출력
